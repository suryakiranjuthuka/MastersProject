...
angular
  ...
  .controller('AppCtrl', function ($rootScope, $location, $scope, $compile, $window, $mdToast) {
    ...
    $rootScope.loginButton = function() {      
      $('#loginPage').css({"display": "block", "opacity": 0});
      $('#loginPage').animate({opacity: 1}, 300);
    ...
    $rootScope.login = function(userName, password) {
      if(userName === "hxu@umassd.edu" && password === "haipingxu"){
        ...
        $('#loginPage').animate({opacity: 0}, 300);
        $('#loginPage').css({"display": "none", "opacity": 0});
  ...
  $('#navTabs a').on("click", function(){
      $(this).css({'border-size': 0+'px'});
      $('#navTabs a').removeClass( "active" );
      $(this).addClass( "active" );
      $('#viewContainer').css({"display": "none", "opacity": 0});
