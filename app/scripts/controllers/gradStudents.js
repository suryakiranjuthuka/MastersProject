...
angular.module( 'cselApp' )
  .controller( 'GradStudentsCtrl', function( GradStudentsContent, $mdDialog, $mdToast, $rootScope ) {
    ...
    GradStudentsContent.get().then( function( data ) {
      var mastersThesisUnder = [],
        mastersProjectUnder = [],
        phdDissertationUnder = [],
        capstoneProjectSupervised = [],
        mastersThesisCommittee = [],
        mastersProjectCommittee = [],
        phdDissertationCommittee = [];

      angular.forEach( data, function( value, key ) {
        if( data[ key ]._type === 'mastersThesisUnder' ) {
          mastersThesisUnder.push( data[ key ] );
        }
        if( data[ key ]._type === 'mastersProjectUnder' ) {
          mastersProjectUnder.push( data[ key ] );
        }
        if( data[ key ]._type === 'phdDissertationUnder' ) {
          phdDissertationUnder.push( data[ key ] );
        }
        if( data[ key ]._type === 'capstoneProjectSupervised' ) {
          capstoneProjectSupervised.push( data[ key ] );
        }
        if( data[ key ]._type === 'mastersThesisCommittee' ) {
          mastersThesisCommittee.push( data[ key ] );
        }
        if( data[ key ]._type === 'mastersProjectCommittee' ) {
          mastersProjectCommittee.push( data[ key ] );
        }
        if( data[ key ]._type === 'phdDissertationCommittee' ) {
          phdDissertationCommittee.push( data[ key ] );
        }
      } );
    ...
    gradStudents.createNew = function( $event, tempType, tempClass, temph2, templabel ) {
      ...
      $mdDialog.show( {
        ...
        template: '<md-dialog class="md-content-overflow">' +
          ...
          '</md-dialog>',
        locals: {
          type: gradStudents.tempType,
          fClass: gradStudents.tempClass,
          temph2: gradStudents.temph2,
          templabel: gradStudents.templabel
        },
    ...
    $rootScope.editOldRG = function( $event, tempType, tempClass, temph2, templabel, tempId ) {
      ...
      $mdDialog.show( {
        ...
        locals: {
          ...
          temph2: gradStudents.temph2,
          templabel: gradStudents.templabel,
          tempId: gradStudents.tempId,
          tempPoint: gradStudents.tempPoint,
          eventTarget: gradStudents.eventTarget
        },
        // controller: 'GreetingController',
